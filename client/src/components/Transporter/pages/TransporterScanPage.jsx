/**
 * TransporterScanPage Component
 * 
 * Dedicated QR scanning page for transporters in the Sentinel supply chain system.
 * 
 * Features:
 ...
 * - Only TRANSPORTER role can scan
...
const TransporterScanPage = ({ shipmentFilter = null, shipmentData = null }) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (shipmentFilter && response.data.shipments) {
          // Try to find matching shipment by shipmentHash, id, or batchId
          ...
          if (filteredShipment) {
            ...
            console.log('Available shipments:', response.data.shipments.map(s => ({
            ...
            if (shipmentData) {
              ...
              setContainerStats({
                ...
                pending: shipmentData.numberOfContainers || 0, // Assume all pending if not found
                scanned: 0,
                isLoading: false
              });
              return;
        ...
        // API failed but we have shipmentData
      ...
      // Fallback to shipmentData if available
      if (shipmentData) {
        setContainerStats({
          total: shipmentData.numberOfContainers || 0,
          pending: shipmentData.numberOfContainers || 0,
  ...
  useEffect(() => {
    // Reset stats to loading state when filter changes
    setContainerStats({
      total: 0,
      pending: 0,
      scanned: 0,
    ...
    // Clear scan history when switching shipments
    setScanHistory([]);
    setScanResult(null);
    setScanState(SCAN_STATES.IDLE);
    setErrorMessage('');
    
    // Fetch new container counts
  ...
  const submitScan = async (containerId, concern) => {
    ...
    try {
      const result = await scanContainerAsTransporter(containerId, { concern });
  ...
  const handleUpdateShipmentStatus = async () => {
    ...
    try {
      // Call API to update shipment status
      const shipmentId = shipmentFilter || shipmentData?.shipmentHash || shipmentData?.id;
      const token = localStorage.getItem('token');
      const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      
      const response = await fetch(`${API_BASE_URL}/api/shipments/${shipmentId}/status`, {
        method: 'PUT',
        headers: {
      ...
      setStatusUpdateError(err.message || 'Failed to update shipment status');
      setIsUpdatingStatus(false);
    }
  };
        ...
        <div className={`border rounded-2xl p-4 ${cardClass}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${isDarkMode ? 'bg-purple-500/10' : 'bg-purple-50'}`}>
              ...
              <div>
                <p className={`text-sm font-medium ${textClass}`}>Containers Scanned</p>
                <p className={`text-xs ${mutedTextClass}`}>
                  {containerStats.pending > 0 
                    ? `${containerStats.pending} remaining to scan`
            ...
            <div className="hidden sm:block w-32">
              ...
              <p className={`text-xs mt-1 text-center ${mutedTextClass}`}>
                {containerStats.total > 0 ? Math.round((containerStats.scanned / containerStats.total) * 100) : 0}% complete
              </p>
            </div>
            {/* Refresh button */}
            <button
      ...
      <div className={`border rounded-2xl p-6 ${cardClass}`}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-lg font-bold ${textClass}`}>Container Scanner</h2>
              <p className={`text-sm ${mutedTextClass}`}>Scan container QR codes for pickup</p>
            </div>
          </div>
        ...
        {scanState === SCAN_STATES.IDLE && (
          <div className="space-y-4">
            {/* Camera Scanner */}
            {scanMode === SCAN_MODES.CAMERA && (
              <div className={`relative rounded-xl overflow-hidden border-2 ${isDarkMode ? 'border-slate-700' : 'border-slate-300'}`}>
                <QRCameraScanner
            ...
            {scanMode === SCAN_MODES.UPLOAD && (
              <div className={`rounded-xl border-2 ${isDarkMode ? 'border-slate-700' : 'border-slate-300'}`}>
                <QRImageScanner
                  onScan={handleQRScanned}
                  onError={(err) => {
        ...
        {scanState === SCAN_STATES.PROCESSING && (
          <div className="flex flex-col items-center justify-center py-12">
            <div className={`w-16 h-16 border-4 rounded-full animate-spin mb-4 ${isDarkMode ? 'border-purple-500/20 border-t-purple-400' : 'border-purple-200 border-t-purple-600'}`} />
        ...
        {scanState === SCAN_STATES.CONCERN_INPUT && (
          <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className={`text-sm ${isDarkMode ? 'text-amber-400/70' : 'text-amber-600'}`}>
                    Add a concern below or skip to proceed with scan
                  </p>
                </div>
              </div>
            </div>
            ...
            <textarea
              ...
              placeholder="Describe any issues (damage, tampering, etc.)..."
              rows={3}
              className={`w-full px-4 py-3 rounded-xl border focus:ring-2 focus:ring-purple-500 focus:border-transparent ${inputClass}`}
            />
          ...
          <div className="space-y-4">
            <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-emerald-50 border border-emerald-200'}`}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className={`text-sm ${isDarkMode ? 'text-emerald-400/70' : 'text-emerald-600'}`}>
                    Container has been marked as IN_TRANSIT
                  </p>
                </div>
              </div>
              ...
              {scanResult.shipment.statusChanged && (
                <div className={`mt-4 p-3 rounded-lg ${isDarkMode ? 'bg-purple-500/10' : 'bg-purple-50'}`}>
            ...
            {statusUpdateSuccess ? (
              ...
              <button
                ...
                className="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-purple-500 text-white font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {isUpdatingStatus ? (
                  <>
              ...
              <button
                ...
                className="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-cyan-500 text-white font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
                <RefreshIcon />
            ...
            {statusUpdateError && (
              <div className={`mt-2 p-3 rounded-lg text-sm text-center ${isDarkMode ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-600'}`}>
                {statusUpdateError}
              </div>
            )}
          </div>
      ...
      <div className={`border rounded-2xl p-6 ${cardClass}`}>
        ...
        <ul className={`text-sm space-y-3 ${mutedTextClass}`}>
          ...
          <li className="flex items-start gap-3">
            ...
            Only containers from blockchain-locked shipments can be scanned
          </li>
          <li className="flex items-start gap-3">
            <span className="text-purple-500 font-bold">3.</span>
            Each container can only be scanned once by a transporter
          </li>
          <li className="flex items-start gap-3">
          ...
          <li className="flex items-start gap-3">
            ...
            Supplier will be notified of any concerns raised
          </li>
        </ul>
      </div>
    </div>
...
export default TransporterScanPage;
