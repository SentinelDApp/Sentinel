...
export const getRejectionMessage = (code) => {
  const messages = {
    ...
    ALREADY_SCANNED: 'This container has already been scanned. Duplicate scans are not allowed.',
...
export const scanContainerAsTransporter = async (containerId, options = {}) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/containers/scan/transporter`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({
        containerId,
...
export const getTransporterAssignedContainers = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/containers/scan/transporter/assigned`, {
      method: 'GET',
      headers: getAuthHeaders()
    });
