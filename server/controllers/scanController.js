...
const verifyScan = async (req, res) => {
  ...
  try {
    ...
    if (parsedQR.type === 'container') {
      ...
      container = await Container.findOne({ containerId: parsedQR.containerId });
      ...
      // Get parent shipment
      shipment = await Shipment.findOne({ shipmentHash: container.shipmentHash });
    } else {
      // Direct shipment lookup
      shipment = await Shipment.findOne({ shipmentHash: parsedQR.shipmentHash });
    }
    ...
    if (!shipment.txHash) {
      ...
      return res.status(400).json({
        ...
        scanId: scanLog.scanId,
        message: 'This shipment has not been locked on the blockchain. The supplier must confirm the shipment before it can be scanned.',
        shipment: {
    ...
    try {
      if (await blockchainService.isAvailable()) {
        const blockchainData = await blockchainService.getShipment(shipment.shipmentHash);
        ...
        if (blockchainData) {
          ...
          if (blockchainData.status !== shipment.status) {
            console.warn(`⚠️  Status mismatch for ${shipment.shipmentHash}: DB=${shipment.status}, Chain=${blockchainData.status}`);
          }
        }
      }
    ...
    if (shipment.status === 'DELIVERED') {
      ...
      return res.status(400).json({
        status: 'REJECTED',
        reason: 'Shipment has already been delivered. No further scans allowed.',
    ...
    return res.status(200).json({
      ...
      nextAction: {
        action: transitionResult.action,
        description: getNextActionDescription(transitionResult.action, user.role),
...
const getScanHistory = async (req, res) => {
  ...
  try {
    const history = await ScanLog.getShipmentScanHistory(shipmentHash, parseInt(limit));
...
const getNextActionDescription = (action, role) => {
  const descriptions = {
    ...
    'WAREHOUSE_ARRIVAL': 'Confirm arrival to update shipment status at warehouse.',
  ...
  try {
    ...
    if (!shipment.txHash) {
      ...
      return res.status(400).json({
        ...
        code: REJECTION_REASONS.NOT_READY_FOR_DISPATCH,
        scanId: scanLog.scanId,
        message: 'This shipment has not been confirmed and locked on the blockchain. The supplier must click "Confirm & Lock Shipment" before containers can be scanned.',
        shipment: {
          shipmentHash: shipment.shipmentHash,
          status: shipment.status,
          isLockedOnBlockchain: false
    ...
    if (container.status === 'DELIVERED') {
      ...
      return res.status(400).json({
        ...
        code: REJECTION_REASONS.ALREADY_DELIVERED,
        scanId: scanLog.scanId,
        message: 'This container has already been delivered. No further scans allowed.',
        container: {
          containerId: container.containerId,
          status: container.status
        }
    ...
    const nextContainerStatus = getNextContainerStatus(role, previousContainerStatus);
...
module.exports = {
  ...
  getScanHistory,
  validateQRFormat,
  scanContainer,       // NEW: Container-specific scan endpoint
