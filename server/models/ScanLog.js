...
scanLogSchema.statics.logAccepted = async function(scanData) {
  ...
  if (containerId) {
    ...
    if (existingScan) {
      console.log(`Container ${containerId} already scanned with action ${action} by ${role}`);
...
/**
 * Log a rejected scan
 * Only saves if this containerId doesn't already have a rejected scan for same action/role
 */
scanLogSchema.statics.logRejected = async function(scanData, reason) {
  ...
  if (containerId) {
    ...
    if (existingScan) {
      console.log(`Container ${containerId} already has scan log for action ${action} by ${role}`);
      existingScan.isDuplicate = true;
      return existingScan;
    }
  }
 ...
 */
scanLogSchema.statics.getShipmentScanHistory = async function(shipmentHash, limit = 50) {
 ...
 */
scanLogSchema.statics.getContainerScanHistory = async function(containerId, limit = 50) {
 ...
 */
scanLogSchema.statics.getActorActivity = async function(walletAddress, limit = 100) {
