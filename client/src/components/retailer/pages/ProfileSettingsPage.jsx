...
function ProfileSettingsContent() {
  ...
  const defaultProfileName = user?.fullName || user?.organizationName || 'Retailer';
        ...
        `}>
          ...
          `}>
            <div className="flex items-center gap-4">
              ...
              `}>
                <UserIcon className={`w-10 h-10 ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`} />
              </div>
              <div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${isDarkMode ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>
                  Local Only
                </span>
              </div>
              ...
              <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                This name is stored locally on your device and won't be synced to the database.
              </p>
              {profileName !== defaultProfileName && (
                <button
                  onClick={handleResetToDefault}
                  className={`mt-2 text-xs ${isDarkMode ? 'text-cyan-400 hover:text-cyan-300' : 'text-cyan-600 hover:text-cyan-700'}`}
                >
            ...
            <div className={`h-px ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-200'}`} />
