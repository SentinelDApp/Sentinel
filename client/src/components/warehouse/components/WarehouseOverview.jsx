import { BoxIcon, TruckIcon, CheckCircleIcon, AlertTriangleIcon } from '../icons/Icons';
import { SHIPMENT_STATUSES } from '../constants';

const WarehouseOverview = ({ shipments, isDarkMode }) => {
  // Calculate stats
  const stats = {
    pending: shipments.filter(s => s.status === SHIPMENT_STATUSES.PENDING).length,
    received: shipments.filter(s => s.status === SHIPMENT_STATUSES.RECEIVED).length,
    verified: shipments.filter(s => s.status === SHIPMENT_STATUSES.VERIFIED || s.status === SHIPMENT_STATUSES.STORED).length,
    concerns: shipments.filter(s => s.status === SHIPMENT_STATUSES.CONCERN_RAISED).length,
  };
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              `}>
                <Icon className={`w-5 h-5 ${isDarkMode ? card.textDark : card.textLight}`} />
              </div>
            </div>
